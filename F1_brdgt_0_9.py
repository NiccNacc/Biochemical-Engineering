...
def r_rct_file(file_path, rtcr_type, ads=[]):
    ...
    rct_idents = ['F1A1','F1A2','F1B1','F1B2','F1C1','F1C2','F1D1','F1D2','50A','50B']
    ...
    # If only accessing one datatype
        ...
        if subplots==True:
            ...
            for i in xydats:
                ...
                label = str(" ") + str(id)
                if plot_t=="line":
                    plt.plot(xydats[ii][0],xydats[ii][1],ls="-",c=c, linewidth="1", label=label)
                if plot_t=="dot":
...
# DG12_proj = batch_open_proj(batch_file_path,'F1','DG12')
# DG12_og = batch_open_merg_og(DG12_proj,og_path=og_path)

# DG20_proj = batch_open_proj(batch_file_path,'F1','DG20')
# DG20_og = batch_open_merg_og(DG20_proj,og_path=og_path)

# DG26_proj = batch_open_proj(batch_file_path,'F1','DG26')
# DG26_og = batch_open_merg_og(DG26_proj,og_path=og_path)
...
# testing_saving_project = r'C:\Users\nmaher\F1workspace'


# save_project(DG12_og,testing_saving_project)
# save_project(DG20_og,testing_saving_project)
# save_project(DG26_og,testing_saving_project)
# testing_b_grap = proj_graphing(DG20_og,"O2")

# testing_b_grap = proj_graphing(DG20_og,["O2","CO2"],subplots=True)

# test_path = r'C:\Users\nmaher\F1workspace'
# DG12_og = open_project("DG12",test_path)
# o_DG20 = open_project("DG20",test_path)
# DG26_og = open_project("DG26",test_path)

# all_projects = [DG26_og]
